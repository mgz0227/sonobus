...
static void DescribeAlgorithmComponent( AAX_IComponentDescriptor * outDesc )
{
	...
	//
	// Register MIDI nodes
	err = outDesc->AddMIDINode( eAlgPortID_MIDINodeIn, AAX_eMIDINodeType_LocalInput, "DemoMIDI In", 0x0001 );
	err = outDesc->AddMIDINode( eAlgPortID_MIDINodeOutPT, AAX_eMIDINodeType_LocalOutput, "DemoMIDI Out", 0x0001 );
	//
	// Register private data
	err = outDesc->AddPrivateData( eAlgPortID_NoteData, sizeof( SDemoMIDI_NoteBits ) );
	//
	// Register meters
...
{
	// Create a new component descriptor for the algorithm component
	AAX_CheckedResult err;
