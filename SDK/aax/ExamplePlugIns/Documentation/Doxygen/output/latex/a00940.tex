\hypertarget{a00940}{}\doxysection{Troubleshooting}
\label{a00940}\index{Troubleshooting@{Troubleshooting}}
How to solve common issues. 

\hypertarget{a00940_troubleshooting_contents}{}\doxysubsection{Contents}\label{a00940_troubleshooting_contents}
\begin{DoxyItemize}
\item \mbox{\hyperlink{a00940_troubleshooting_loading}{Plug-\/\+In Fails to Load in Shipping Pro Tools}} \item \mbox{\hyperlink{a00940_troubleshooting_performance}{Plug-\/\+In Causes Audio Streaming Errors}}\end{DoxyItemize}
\hypertarget{a00940_troubleshooting_loading}{}\doxysubsection{Plug-\/\+In Fails to Load in Shipping Pro Tools}\label{a00940_troubleshooting_loading}
...
To verify whether this failure is due to an invalid digital signature vs. some other library loading failure, check the Pro Tools \mbox{\hyperlink{a00932}{log file}}. A failure caused by an invalid digital signature will result in log lines like the following\+:
...
Another way to check whether a plug-\/in\textquotesingle{}s digital signature is invalid is to test the plug-\/in in a Pro Tools developer build or with the \mbox{\hyperlink{a00933}{Digi\+Shell}} utility. If the plug-\/in successfully loads and runs in these tools but not in a shipping build of Pro Tools then it is very likely that the problem is in the plug-\/in\textquotesingle{}s digital signature.
...
If the failure is occurring only one certain systems then try archiving and copying the failing plug-\/ins back to a system where the plug-\/in loads successfully then comparing the archived copy to a known successful copy to see if there are any differences to the file names or binary contents of the files within the bundle.\hypertarget{a00940_troubleshooting_performance}{}\doxysubsection{Plug-\/\+In Causes Audio Streaming Errors}\label{a00940_troubleshooting_performance}
...
In an AAX plug-\/in, parameter changes are applied on a different thread than audio processing. AAX includes a system for synchronizing delivery of parameter updates to the algorithm at the correct time without requiring any synchronization by the plug-\/in, and you are strongly encouraged to understand the details of how \mbox{\hyperlink{a00918}{parameter updates}} work, in particular how the \mbox{\hyperlink{a00919}{parameter update timing}} is achieved and how to implement proper timing and synchronization even in a plug-\/in that does not use a standard decoupled algorithm callback.
