...
static void DescribeAlgorithmComponent( AAX_IComponentDescriptor* outDesc )
{
	...
	err = properties->AddProperty ( AAX_eProperty_ProductID, cDemoGainRelated_ProductID );
	...
	//
	// ID properties
	err = properties->AddProperty ( AAX_eProperty_PlugInID_Native, cDemoGainRelated_PlugInID_Native );
	err = properties->AddProperty ( AAX_eProperty_PlugInID_AudioSuite, cDemoGainRelated_PlugInID_AudioSuite );	// for offline processing
	err = properties->AddProperty ( AAX_eProperty_PlugInID_TI, cDemoGainRelated_PlugInID_TI );
	
	// Explicit product relationship
	{
		AAX_SPlugInIdentifierTriad relatedNativeID;
		relatedNativeID.mManufacturerID = cDemoGain_ManufactureID;
		relatedNativeID.mProductID = cDemoGain_ProductID;
		relatedNativeID.mPlugInID = cDemoGain_PlugInID_Native;
		err = properties->AddPropertyWithIDArray(AAX_eProperty_Related_Native_Plugin_List, &relatedNativeID, 1);
	}
	{
		AAX_SPlugInIdentifierTriad relatedDSPID;
		relatedDSPID.mManufacturerID = cDemoGain_ManufactureID;
		relatedDSPID.mProductID = cDemoGain_ProductID;
		relatedDSPID.mPlugInID = cDemoGain_PlugInID_TI;
		err = properties->AddPropertyWithIDArray(AAX_eProperty_Related_DSP_Plugin_List, &relatedDSPID, 1);
	}
...
static AAX_Result DescribeEffect( AAX_IEffectDescriptor* outDescriptor )
{
	...
	// Effect identifiers
	//
	err = outDescriptor->AddName ( "Demo Gain Related Example" );
	err = outDescriptor->AddName ( "Demo Gain Related" );
	err = outDescriptor->AddName ( "DemoGain Rel" );
	err = outDescriptor->AddName ( "DmGain Rel" );
	err = outDescriptor->AddName ( "DG Rel" );
	err = outDescriptor->AddName ( "DGRl" );
	err = outDescriptor->AddName ( "DR" );
...
AAX_Result GetEffectDescriptions( AAX_ICollection* outCollection )
{
	...
	if ( plugInDescriptor )
	{
		AAX_SWALLOW_MULT(
			...
			err = outCollection->AddEffect( kEffectID_DemoGain_Related, plugInDescriptor );
		);
	}
	else
		err = AAX_ERROR_NULL_OBJECT;
	...
	err = outCollection->AddPackageName( "DemoGain Related AAX SDK Example Plug-In" );
	err = outCollection->AddPackageName( "DemoGain Related" );
	err = outCollection->AddPackageName( "DmG Rel" );
	err = outCollection->AddPackageName( "DmGR" );
